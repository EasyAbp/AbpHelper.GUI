...
let consoleNode = document.getElementById('box-abphelper-cli-ef-migrations-add').getElementsByTagName('textarea')[0]

const execBtn = document.getElementById('ef-migrations-add-execute')
const selectSolutionFileBtn = document.getElementById('ef-migrations-add-select-solution-file-btn')
const extraOptionsCheckBox = {
  noOverwrite: document.getElementById('ef-migrations-add-options-noOverwrite')
}
...
selectSolutionFileBtn.addEventListener('click', (event) => {
  dialog.showOpenDialog({
    ...
    if (result.filePaths[0]) {
      document.getElementById('ef-migrations-add-solution-file').value = result.filePaths[0]
    }
  }).catch(err => {
    console.log(err)
  })
...
function runExec() {
  ...
  document.getElementById('ef-migrations-add-process').style.display = 'block'
